...
export default [
  {
    ...
    // is not reformatted on the way in. It is ignored because it is not ours to edit:
    // today it happens to produce zero lint errors (checked with `eslint --no-ignore`,
    // not assumed), so this is not paying off a debt, it is refusing to take one on the
    // next time the generator's output shape moves. It is still type-checked —
