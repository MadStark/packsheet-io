...
**A row's state is shown before the name or through it and, for a row that failed outright, at its left edge and across the row too — never in the middle and never off the sheet:**

- an inline 16px **status icon** before the name, carrying its own accessible name
- a **strike** through a retired item's name
- for a row that is genuinely exceptional — one that _failed_, rather than one that is merely a different kind of ordinary — a **5.5% wash** of the state's colour across the row and a **3px bar** of that colour at its left edge

**The wash and the bar no longer mark the closet's gear statuses, and this is written as an amendment (PK-70)** rather than made as a quiet deletion, because this section prescribed them for exactly that case for months and the closet shipped with them. Design feedback found the rail noisy, and the reason it read as noise is that it was marking the wrong kind of thing: two of the three gear statuses are ordinary states of a closet rather than faults, so a coloured rail down a third of the list is repeated colour carrying no urgency — the same argument the unmarked-default rule below already makes about owned rows, one step further out. What carries a non-owned status now is the icon and the strike, which is shape rather than hue and is what §12 actually asks for. The wash and the bar stay in the language for the case that suits them: `.row-invalid` draws them in rust for a row that could not be imported, which is §9's alert treatment applied to a row instead of to a note.
...
**Colour is never the only signal.** A row in a state carries an icon with an accessible name, or a strike — shape as well as hue. The closet's coloured rail is gone (§7, PK-70) and this rule is why nothing was lost with it: the rail was the hue half, and the half that had to survive did. This matters more now than it used to: the index tab, which carried a _word_, is gone.
...
| Index tabs hang off the sheet's right edge                       | **Removed**                                       | The last thing on the page that overlapped another paper. Its accessible word is replaced by the row's status icon and strike (the wash and bar it also named were dropped for gear statuses by PK-70 — see the row below, and §7). |
...
| No two papers overlap, without exception                         | **A modal may cover the page**                    | PK-69. The absolute form refused the popup the design feedback asked for in five places. Narrowed rather than dropped: overlap for _emphasis_ is still refused, and a modal is not part of the page.                                |
| A row's status is a wash and a left bar                          | **Icon and strike only, in the closet**           | PK-70. Two of the three gear statuses are ordinary rather than exceptional, so a coloured rail down a third of the list marked the wrong kind of thing. The wash and bar stay for a row that actually failed.                       |
